...
Path_series = os.path.abspath(os.path.join(os.path.dirname(__file__), 'CleanData/PPT'))
Path_IDF    = os.path.abspath(os.path.join(os.path.dirname(__file__), 'IDF'))

if os.path.exists(Path_IDF) == False:
    os.makedirs(Path_IDF)
...
def MaxAnual(Esta, Path_series, window=None):
    ...
    """
    try:
        Dat = Read.EstacionCSV_pd(Esta, Esta.split('.csv')[0],Path_series)
    except:
